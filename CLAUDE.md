...
- **Readability**: Avoid boolean parameters - use descriptive wrapper functions
  - Bad: `testStatement(t, input, want, true)` - unclear what `true` means
  - Good: `testStatementTypeOnly(t, input, want)` - intent is clear
- **Signal-to-Noise Ratio**: Test intent should be immediately obvious
...
- **Helper Functions**: Must be genuinely reusable (used in multiple places)
- **Pointer-Based Expectations**: Use nil to skip checks in test structs
  ```go
