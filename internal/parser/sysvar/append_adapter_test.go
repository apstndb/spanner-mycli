...
func TestProtoDescriptorFileParser(t *testing.T) {
	...
	t.Run("Append operation with GoogleSQL mode", func(t *testing.T) {
		...
		appendable, ok := p.(sysvar.AppendableVariableParser)
		if !ok {
			t.Fatal("Parser does not implement AppendableVariableParser")
		}
		
		err := appendable.AppendWithMode(`"new.proto"`, parser.ParseModeGoogleSQL)
		if err != nil {
	...
	t.Run("Append operation with Simple mode", func(t *testing.T) {
		...
		err := appendable.AppendWithMode("another.proto", parser.ParseModeSimple)
		if err != nil {
			t.Fatalf("AppendWithMode failed: %v", err)
		}
