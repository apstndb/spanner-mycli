...
func (s *BeginRwStatement) Execute(ctx context.Context, session *Session) (*Result, error) {
	...
	if err := session.txn.BeginReadWriteTransaction(ctx, s.IsolationLevel, s.Priority); err != nil {
		return nil, err
	}
...
func (s *BeginStatement) Execute(ctx context.Context, session *Session) (*Result, error) {
	...
	if session.systemVariables.Transaction.ReadOnly {
		ts, err := session.txn.BeginReadOnlyTransaction(ctx, timestampBoundUnspecified, 0, time.Time{}, s.Priority)
		if err != nil {
			return nil, err
		}
	...
	err := session.txn.BeginPendingTransaction(ctx, s.IsolationLevel, s.Priority)
	if err != nil {
		return nil, err
	}
...
func (s *SetTransactionStatement) Execute(ctx context.Context, session *Session) (*Result, error) {
	...
	if s.IsReadOnly {
		ts, err := session.txn.BeginReadOnlyTransaction(ctx, timestampBoundUnspecified, 0, time.Time{}, attrs.priority)
		if err != nil {
			return nil, err
		}
		...
		err := session.txn.BeginReadWriteTransaction(ctx, attrs.isolationLevel, attrs.priority)
		if err != nil {
			return nil, err
		}
		return result, nil
...
func (s *BeginRoStatement) Execute(ctx context.Context, session *Session) (*Result, error) {
	...
	ts, err := session.txn.BeginReadOnlyTransaction(ctx, s.TimestampBoundType, s.Staleness, s.Timestamp, s.Priority)
	if err != nil {
		return nil, err
	}
