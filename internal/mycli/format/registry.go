...
func unregisterStreamingFormatter(modes ...Mode) {
	registryMu.Lock()
	defer registryMu.Unlock()
	for _, mode := range modes {
		delete(streamingFmtRegistry, mode)
	}
}

// unregisterValueFormatMode removes a registered ValueFormatMode. Used by tests to clean up.
func unregisterValueFormatMode(modes ...Mode) {
	registryMu.Lock()
	defer registryMu.Unlock()
	for _, mode := range modes {
		delete(valueFormatRegistry, mode)
	}
}
