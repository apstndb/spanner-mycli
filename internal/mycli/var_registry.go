...
func (r *VarRegistry) registerAll() {
	...
	r.Register("CLI_BIGQUERY_PROJECT", StringVar(&sv.Feature.BigQueryProject, "GCP project for BigQuery queries. Defaults to CLI_PROJECT when empty."))
	r.Register("CLI_BIGQUERY_LOCATION", StringVar(&sv.Feature.BigQueryLocation, "BigQuery location for queries (e.g. US, EU)."))
	r.Register("CLI_BIGQUERY_MAX_BYTES_BILLED", NullableIntVar(&sv.Feature.BigQueryMaxBytesBilled, "Maximum bytes billed per BigQuery query."))
