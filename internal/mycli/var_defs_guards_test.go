...
func TestLocalAllowedRoundTrip(t *testing.T) {
	...
	for i := range varDefs {
		...
		t.Run(def.name, func(t *testing.T) {
			...
			// Unimplemented placeholders are nominally localAllowed but reject
			// both Get and Set until their runtime behavior lands. SET LOCAL on
			// them fails at the SET itself, so no undo entry is ever recorded
			// and there is no restore hazard.
