...
func TestBuildStatement(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			desc:          "DUMP TABLES LIKE one pattern",
			input:         "DUMP TABLES LIKE 'User%'",
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Like: []string{"User%"}}},
			skipLowerCase: true,
		},
		{
			desc:          "DUMP TABLES LIKE multiple patterns",
			input:         "DUMP TABLES LIKE 'User%', 'Tmp%'",
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Like: []string{"User%", "Tmp%"}}},
			skipLowerCase: true,
		},
		{
			desc:          "DUMP TABLES EXCEPT patterns",
			input:         "DUMP TABLES EXCEPT 'Tmp%', 'Test%'",
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Except: []string{"Tmp%", "Test%"}}},
			skipLowerCase: true,
		},
		{
			desc:          "DUMP TABLES LIKE and EXCEPT",
			input:         "DUMP TABLES LIKE 'User%' EXCEPT 'UserTest%'",
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Like: []string{"User%"}, Except: []string{"UserTest%"}}},
			skipLowerCase: true,
		},
		{
			desc:          "DUMP TABLES LIKE empty string is a real pattern",
			input:         "DUMP TABLES LIKE ''",
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Like: []string{""}}},
			skipLowerCase: true,
		},
		{
			desc:          "DUMP TABLES LIKE raw string keeps backslash",
			input:         `DUMP TABLES LIKE r'a\_b'`,
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Like: []string{`a\_b`}}},
			skipLowerCase: true,
		},
		{
			desc:          "DUMP TABLES LIKE escaped SQL string",
			input:         `DUMP TABLES LIKE 'a\\_b'`,
			want:          &DumpTablesStatement{Selector: &dumpTableSelector{Like: []string{`a\_b`}}},
			skipLowerCase: true,
		},
		{
			desc:  "DUMP TABLES quoted LIKE is an exact table name",
			input: "DUMP TABLES `LIKE`",
			want:  &DumpTablesStatement{Tables: []tableID{{Name: "LIKE"}}},
		},
		{
			desc:  "DUMP TABLES quoted EXCEPT is an exact table name",
			input: "DUMP TABLES `EXCEPT`",
			want:  &DumpTablesStatement{Tables: []tableID{{Name: "EXCEPT"}}},
		},
		{
		...
		{
			desc:  "SAVEPOINT statement",
			input: "SAVEPOINT keep",
			want:  &SavepointStatement{Name: "keep"},
		},
		{
			desc:          "SAVEPOINT quoted identifier preserves case",
			input:         "SAVEPOINT `KeepMe`",
			want:          &SavepointStatement{Name: "KeepMe"},
			skipLowerCase: true,
		},
		{
			desc:  "ROLLBACK TO statement",
			input: "ROLLBACK TO keep",
			want:  &RollbackToSavepointStatement{Name: "keep"},
		},
		{
			desc:  "ROLLBACK TO SAVEPOINT statement",
			input: "ROLLBACK TO SAVEPOINT keep",
			want:  &RollbackToSavepointStatement{Name: "keep"},
		},
		{
			desc:  "ROLLBACK TRANSACTION TO SAVEPOINT statement",
			input: "ROLLBACK TRANSACTION TO SAVEPOINT keep",
			want:  &RollbackToSavepointStatement{Name: "keep"},
		},
		{
			desc:  "RELEASE statement",
			input: "RELEASE keep",
			want:  &ReleaseSavepointStatement{Name: "keep"},
		},
		{
			desc:  "RELEASE SAVEPOINT statement",
			input: "RELEASE SAVEPOINT keep",
			want:  &ReleaseSavepointStatement{Name: "keep"},
		},
		{
...
func TestBuildStatement_InvalidCase(t *testing.T) {
	...
	invalidInputs := []string{
		...
		"DUMP TABLES LIKE 'User%' EXCEPT",
		"DUMP TABLES LIKE 'User%' leftover",
		"DUMP TABLES EXCEPT 'Tmp%' LIKE 'User%'",
		"DUMP TABLES LIKE Users",
		"DUMP TABLES LIKE User%",
		"DUMP TABLES t1 LIKE 'User%'",
		"DUMP TABLES LIKE 'User%' t1",
