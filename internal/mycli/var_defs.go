...
var varDefs = []varDef{
	...
	{
		name:    "AUTOCOMMIT",
		desc:    "A boolean indicating whether the connection is in autocommit mode. TRUE (default) uses single-use reads and implicit RW DML commits. FALSE lazily starts a pending logical owner at the next eligible database operation (ordinary SELECT/DML/MUTATE, EXPLAIN ANALYZE, automatic DML enqueue, nonempty RUN BATCH DML, or first enabled SAVEPOINT) and stays idle after COMMIT/ROLLBACK. Same-value SET/RESET is idempotent even with an owner or manual batch; a real change is rejected with the existing transaction/batch setter errors. SET LOCAL is not supported. false→true never implicit-Commits. EXPLAIN PLAN, DDL, PDML/TRUNCATE, partition, admin, and inspection commands do not create a new owner. EOF/EXIT/Close never commit unfinished work. Idle expiry is #357.",
		scope:   scopeSession,
		noLocal: true,
