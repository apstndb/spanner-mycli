...
func TestDumpStatements(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute DUMP statement
			result, err := tt.stmt.Execute(ctx, session)
			if err != nil {
				t.Fatalf("Execute failed: %v", err)
			}

			// Check that IsDirectOutput is true
...
func TestDumpTablesWithInvalidTable(t *testing.T) {
	...
	// Try to dump non-existent table
...
func TestDumpEmptyDatabase(t *testing.T) {
	...
	// DUMP DATABASE on empty database should work (DDL only)
	stmt := &DumpDatabaseStatement{}
	...
	// Should have at least the header comment
...
func TestDumpWithStreaming(t *testing.T) {
	...
	// Insert test data
	for i := 1; i <= 5; i++ {
		dml := fmt.Sprintf("INSERT INTO StreamTest (id, value) VALUES (%d, 'value%d')", i, i)
		stmt, err := BuildStatement(dml)
		if err != nil {
			t.Fatalf("Failed to build DML statement: %v", err)
		}
		if _, err := stmt.Execute(ctx, session); err != nil {
	...
	// Check that result indicates streaming was used
	if !result.Streamed {
