...
| Export specific tables as SQL INSERT statements                             | `DUMP TABLES <table1> [, <table2>, ...];`                                                                  | Table names are [<schema>.]<table>. Data only; no DDL. Invalid names are rejected before GetDatabaseDdl. Requires spanner.databases.getDdl only when a selected interleaved child has another selected BASE TABLE whose name matches the catalog parent basename. Populated cyclic enforced foreign-key graphs among the selected tables are rejected before any output (interim). |
