...
| Manipulate PROTO BUNDLE                                                     | `SYNC PROTO BUNDLE [{UPSERT\|DELETE} (<type> ...)]...;`                                                    | Mixed UPSERT and DELETE clauses are one operation. Duplicate names within one operation are kept once in first-occurrence order. The same name in both UPSERT and DELETE is rejected. A composed `ALTER PROTO BUNDLE` may include INSERT, UPDATE, and DELETE together; that is not a general atomic DDL-batch or schema-read guarantee. |
