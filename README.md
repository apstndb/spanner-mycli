...
| Export the last cached query plan as official Cloud Spanner ProtoJSON | `SHOW LAST QUERY PLAN [WITH STATS] [INTO <path>];`                                                         | Writes a QueryPlan (or ResultSetStats with WITH STATS). Requires a preceding query or EXPLAIN ANALYZE. Intended for external plan viewers; not a stable versioned contract.               |
