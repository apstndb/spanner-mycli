...
| `CLI_PROTO_DESCRIPTOR_FILE`       | read,write,add | Comma-separated list of proto descriptor files. Supports ADD to append files. HTTP(S) source vs binary is classified from the URL path, not query or fragment.                                                                                                                                                                                                                                                                                                                                                            |
...
| `DIRECTED_READ`                   | read,write     | Directed read options for supported read-only queries, as replica_location or replica_location:READ_ONLY\|READ_WRITE. Empty string clears. SET is rejected while a transaction is pending or active; SET LOCAL is not supported. Not applied to read-write queries, DML, heartbeat, or partitioned DML. This is not JDBC protobuf JSON.                                                                                                                                                                                   |
...
#### DIRECTED_READ
- **Type**: STRING (`location` or `location:READ_ONLY|READ_WRITE`)
...
- **Description**: Routes supported read-only queries to a replica selection. The same grammar as `--directed-read`. An empty value clears the option. SHOW omits `TYPE_UNSPECIFIED` so a region-only value round-trips.
- **Notes**:
  - Not JDBC protobuf JSON. java-spanner `SET directed_read` accepts compact JSON; this CLI keeps the location[:type] shorthand.
  ...
  - One include selection with auto-failover disabled, matching `--directed-read`.
