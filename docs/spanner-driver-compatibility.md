...
|----------|--------------------------|------------------------|----------------------|
| `retry_aborts_internally` | yes (pre-v1.0; default `true`) | yes; `SET LOCAL retry_aborts_internally` support in v6.83.0 (2024-12-13) | `RETRY_ABORTS_INTERNALLY` implemented for implicit autocommit read-write only (#994, part of #293). Opt-in default `FALSE` (Java/Go default `TRUE`). Application logical retry of ABORTED SQL/BatchDML/Commit, at most 50 attempts including the first, reusing the original cancellation and `TRANSACTION_TIMEOUT` budget. Not SDK gRPC/gax/stream retries. Session SET/RESET applies to later owners; SHOW is the session value. SET LOCAL is uniformly unsupported in this phase (TRUE and FALSE) with an actionable error; later explicit journal replay will add pending-owner LOCAL. Explicit/pending RW owners, including `AUTOCOMMIT=false`, are rejected while TRUE. PLAN, PDML, and RO are not new retry paths. This is not complete #293 parity or a stable driver contract. |
