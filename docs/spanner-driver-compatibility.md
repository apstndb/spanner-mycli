...
|----------|--------------------------|------------------------|----------------------|
| `retry_aborts_internally` | yes (pre-v1.0; default `true`) | yes; `SET LOCAL retry_aborts_internally` support in v6.83.0 (2024-12-13) | `RETRY_ABORTS_INTERNALLY` implemented for implicit autocommit read-write (#994) and explicit read-write journal replay (#1006, part of #293). Opt-in default `FALSE` (Java/Go default `TRUE`). Application logical retry of ABORTED SQL/DML/THEN RETURN/PROFILE/Batch DML/Commit, at most 50 physical attempts including the first per logical owner (implicit: one operation; explicit: the whole transaction, including prefix-replay ABORTED), reusing the original cancellation and `TRANSACTION_TIMEOUT` budget. Not SDK gRPC/gax/stream retries. Session SET/RESET applies to later owners; SHOW is the session value; an existing owner keeps its captured value. SET LOCAL TRUE/FALSE is allowed only on an unused pending read-write owner before the first RPC, queued work, or SAVEPOINT marker. Direct `BEGIN RW` and implicit RW have no LOCAL window. Explicit retry silently replays the fully observed prefix and retries the current operation; bytes already delivered by that current operation, writer failures, PLAN-only, PDML, RO, and non-ABORTED Commit failures are not retried. A changed replay result ends the logical transaction. Hidden reads and unreturned volatile writes are not preserved. This is not a stable driver contract. |
...
| `transaction_timeout` | yes (v1.22.0) | v6.101.0 | `TRANSACTION_TIMEOUT` implemented. Duration or `NULL`; `NULL`/0 means no additional logical read/write deadline. The duration is captured for the logical owner and the single total budget starts at the first real database RPC (including constructor `BeginTransaction`), not client-only `BEGIN`/`SHOW` or DML buffering. The budget is preserved across physical reconstruction, including implicit (#994) and explicit (#1006) ABORTED retry. Pending `SET LOCAL` may select the duration before the first RPC; changing it after activation is rejected. Session `SET` after `BEGIN` applies to a later owner. Distinct from `STATEMENT_TIMEOUT` and `CLI_IDLE_TRANSACTION_TIMEOUT` (#357). |
...
| `CLI_IDLE_TRANSACTION_TIMEOUT` | n/a (CLI-owned) | n/a (not a JDBC property) | Implemented (#357). Sliding user-idle quiet interval; `NULL`/0 disabled; no default 60s. Captured at `BEGIN`. Successful explicit `BEGIN RW`/`BEGIN RO` starts the first quiet interval. Completed admitted work rearms, including buffered DML/`MUTATE` and SAVEPOINT ops. Heartbeat and client-only `SHOW`/`SET` do not reset it. Independent of `KEEP_TRANSACTION_ALIVE`. Not an RPC deadline. Distinct from `CLI_DDL_IN_TRANSACTION_MODE` `hasUserWork`. |
